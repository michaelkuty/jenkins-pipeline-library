...
class Project {
    ...
    def cleanOldPullRequests(manager, howOld=7) {
        ...
        for ( folder in prs_folder.getItems() ) {
            ...
            if ( last_build == null ) {
                def gh_pr = this.getAuthenticatedRepository().getPullRequest(folder.getName().toInteger());
                if ( gh_pr.isMerged() ) {
                    manager.listener.logger.println "No last build for ${folder_main_build.getFullDisplayName()} but PR is merged. Deleting folder"
                    folder.delete()
                } else {
            ...
            if ( last_build_date < current_date ) {
                ...
                def workspace = last_build.getWorkspace().absolutize().getParent();
                folder.delete()
                // Let's make sure the associated workspace get's deleted
                if ( workspace.exists() ) {
